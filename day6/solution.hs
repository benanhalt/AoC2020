...
main = do
  putStrLn "Part 1:"
  let groups = drop 1 <$> groupBy (\a b -> b /= "") input
  -- putStrLn $ show $ groups
  putStrLn $ show $ foldl (+) 0 $ length <$> nub <$> concat <$> groups

  putStrLn "Part 2:"
  putStrLn $ show $ foldl (+) 0 $ length <$> (\group -> foldl intersect (head group) (tail group)) <$> groups
