...
requiredKeys = words "byr iyr eyr hgt hcl ecl pid"

main :: IO ()
main = do
  putStrLn "Part 1:"
  let records = drop 1 <$> groupBy (\a b -> b /= "") input
  let passports = (items <$> records)
  let validPassports = filter valid passports
  putStrLn $ show $ length  validPassports

  putStrLn "Part 2:"
  let reallyValidPassports = filter (\p -> all validItem p) validPassports
  putStrLn $ show $ length  reallyValidPassports


validItem :: (String, String) -> Bool
validItem (key, value) = case key of
  "byr" -> (all isDigit value) && (1920 <= year) && (year <= 2002)
    where
      year = read value

  "iyr" -> (all isDigit value) && (2010 <= year) && (year <= 2020)
    where
      year = read value

  "eyr" -> (all isDigit value) && (2020 <= year) && (year <= 2030)
    where
      year = read value
  ...
  "hgt" -> case units of
    ...
    _ -> False
    where
      units = dropWhile isDigit value
      v = read $ takeWhile isDigit value

  "hcl" -> (head value == '#') && (all (`elem` "1234567890abcdef") $ drop 1 value)

  "ecl" -> value `elem` (words "amb blu brn gry grn hzl oth")
...
items ss = do
  s <- ss
  w <- words s
  let [key, value] = words ((\c -> if c == ':' then ' ' else c) <$> w)
  [(key, value)]

valid :: [(String, String)] -> Bool
valid items = length (keys `intersect` requiredKeys) == length requiredKeys
  where
    keys = fst <$> items
