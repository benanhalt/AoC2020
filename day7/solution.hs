...
main = do
  putStrLn "Part 1:"
  let rules = containment <$> input
  let canContain = rContainersOf rules "shiny gold"
  putStrLn $ show $ length $ nub $ canContain

  putStrLn "Part 2:"
  putStrLn $ show $ rContain rules "shiny gold"

contain :: [(String, String)] -> String -> [(Int, String)]
contain rules container = if contains == "contain no other bags." then []
                          else parseBags <$> splitOn "," contains
  where
    Just contains = (container ++ " bags") `lookup` rules

rContain :: [(String, String)] -> String -> Int
rContain rules container = foldl (+) 0 foo
  where
    foo = do
      (n, container') <- contain rules container
      pure $ n * (1 + rContain rules container')

parseBags :: String -> (Int, String)
parseBags s = (read count, adj ++ " " ++ clr)
  where
    ws = words s
    [count, adj, clr, _] = drop (length ws - 4) ws


rContainersOf :: [(String, String)] -> String -> [String]
rContainersOf rules inner = do
  let cso = containersOf rules inner
  co <- cso
  cso ++ rContainersOf rules co


containersOf :: [(String, String)] -> String -> [String]
containersOf rules inner = do
  (container, contains) <- rules
  guard $ inner `isInfixOf` contains
  pure $ unwords $ take 2 $ words $ container

containment :: String -> (String, String)
containment s = (unwords outer, unwords inner)
  where
    (outer, inner) = span (/= "contain") $ words s
